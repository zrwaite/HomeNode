...
interface userPostBody { //Body query interface for post request
    username: string;
    name: string;
    home_id: string;
    settings: object;
}
interface userPutBody { //Body query interface for put request
    name?: string;
...
export default class userController {
    static async apiGetUser(req:Request, res: Response, next: NextFunction) {
        ...
        if (query.id){ //Find by id
            try{user = await User.findById(query.id);} 
    ...
    static async apiPutUser(req:Request, res: Response, next: NextFunction) {
        ...
        switch (type) {
            case "name":
                try {
                    user = await User.findByIdAndUpdate(id, {name: body.name}, {new:true}); //Saves branch to mongodb
                } catch (e:any) {result.errors.push("Error creating name request", e);}
                break;
            case "dark_mode":
                try {
                    user = await User.findByIdAndUpdate(id, {"settings.dark_mode": body.settings?.dark_mode}, {new:true}); //Saves branch to mongodb
                } catch (e:any) {result.errors.push("Error creating settings darkMode request", e);}
                break;
            case "email_notifications":
                try {
                    user = await User.findByIdAndUpdate(id, {"settings.email_notifications": body.settings?.email_notifications}, {new:true}); //Saves branch to mongodb
                } catch (e:any) {result.errors.push("Error creating settings email notifications request", e);}
                break;
            default:
